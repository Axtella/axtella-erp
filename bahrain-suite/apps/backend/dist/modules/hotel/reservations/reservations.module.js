"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.HotelReservationsModule = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const hotel_room_entity_1 = require("../rooms/entities/hotel-room.entity");
const hotel_reservation_entity_1 = require("./entities/hotel-reservation.entity");
const reservations_controller_1 = require("./reservations.controller");
const reservations_service_1 = require("./reservations.service");
let HotelReservationsModule = class HotelReservationsModule {
};
exports.HotelReservationsModule = HotelReservationsModule;
exports.HotelReservationsModule = HotelReservationsModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([hotel_reservation_entity_1.HotelReservation, hotel_room_entity_1.HotelRoom])],
        controllers: [reservations_controller_1.HotelReservationsController],
        providers: [reservations_service_1.HotelReservationsService],
        exports: [reservations_service_1.HotelReservationsService],
    })
], HotelReservationsModule);
//# sourceMappingURL=reservations.module.js.map